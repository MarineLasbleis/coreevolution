...
# Core conductivity (estimated) #change between 60,150,or250
k_c = 150.
...
# ------------------------------------------------------------------------------------------------------------------- #
        ...
        if self.planet.r_IC_0 == 0.0:
            ...
            self.P_IC[0] = self.pressure_diff(
                self.planet.r_IC_0)+self.planet.P0
            self.S_t[0] = self.planet.S * \
    ...
    def run(self):
        ...
        for i, time in enumerate(self.planet.time_vector[1:]):
            ...
            if self.r_IC[i] == 0.0 and self.T[i] > self.planet.TL0:
                ...
                T, dT_dt, r_IC, drIC_dt, PC, PL, PX, Q_CMB, T_CMB, QC, QL, QX, qc_ad, F_th, F_X, Bc, Bs, M, M_ratio, P_IC, S_t, M_Aubert = self.update_noic(
                    self.T[i], self.Delta_time[i+1], self.planet.qcmb[i])
                # Shift updated value to the next time step"""
                T, r_IC, drIC_dt, PC, PL, PX, Q_CMB, T_CMB, QC, QL, QX, qc_ad, F_th, F_X, Bc, Bs, M, M_ratio, P_IC, S_t, M_Aubert = self.update_value(
                    T, r_IC, drIC_dt, PC, PL, PX, Q_CMB, T_CMB, QC, QL, QX, qc_ad, F_th, F_X, Bc, Bs, M, M_ratio, P_IC, S_t, M_Aubert, i)
                assert abs(Q_CMB-self.planet.qcmb[i]*self.planet.r_OC**2 * 4 * np.pi) < 1., (
                    Q_CMB/1e13, (self.planet.qcmb[i]*self.planet.r_OC**2 * 4 * np.pi)/1e13)
                ...
                if self.T[i+1] < self.planet.TL0:
                    ...
                    for m in range(timesteps):
                        ...
                        T, r_IC, drIC_dt, PC, PL, PX, Q_CMB, T_CMB, QC, QL, QX, qc_ad, F_th, F_X, Bc, Bs, M, M_ratio, P_IC, S_t, M_Aubert = self.update_ic(
                            r_IC_0, dt, self.planet.qcmb[i], P_IC_0, self.planet.S, ratio=ratio_0)
                        ...
                        if m == timesteps-1:
                            Q_CMB = (sum_ratio+ratio)*4*np.pi * \
                                self.planet.qcmb[i]*self.planet.r_OC**2
                ...
                for m in range(timesteps):
                    ...
                    T, r_IC, drIC_dt, PC, PL, PX, Q_CMB, T_CMB, QC, QL, QX, qc_ad, F_th, F_X, Bc, Bs, M, M_ratio, P_IC, S_t, M_Aubert = self.update_ic(
                        r_IC, dt, self.planet.qcmb[i], self.P_IC[i], self.S_t[i], ratio=ratio_0)
                    Q_CMB_0 += Q_CMB
                    T, r_IC, drIC_dt, PC, PL, PX, Q_CMB, T_CMB, QC, QL, QX, qc_ad, F_th, F_X, Bc, Bs, M, M_ratio, P_IC, S_t, M_Aubert = self.update_value(
                        T, r_IC, drIC_dt, PC, PL, PX, Q_CMB, T_CMB, QC, QL, QX, qc_ad, F_th, F_X, Bc, Bs, M, M_ratio, P_IC, S_t, M_Aubert, i)
                assert abs(Q_CMB-(sum_ratio)*self.planet.qcmb[i]*self.planet.r_OC**2 * 4 * np.pi) < 1., (
                    Q_CMB/1e13, (self.planet.qcmb[i]*self.planet.r_OC**2 * 4 * np.pi)/1e13)
# ------------------------------------------------------------------------------------------------------------------- #
...
# ------------------------------------------------------------------------------------------------------------------- #
    def plot(self, plots_folder):
        ...
        ax2 = ax1.twinx()
        ax2.plot(self.planet.time_vector, self.r_IC /
                 self.planet.r_OC, color='teal')
        ax2.set_ylabel('Inner core radius fraction', color='teal')
        ax2.tick_params(axis='y', labelcolor='teal')
        plt.savefig(plots_folder + 'T+r_IC_{}ME_{}XFe_{}FeM.pdf'.format(
            self.planet.Mp, self.planet.XFe, self.planet.FeM), bbox_inches="tight")
        plt.show()
        ...
        if self.planet.S > 0:
            ...
            plt.xlim([0, 5e9])
            # plt.ylim([4,7.])
            plt.savefig(plots_folder + 'LE_{}ME_{}XFe_{}FeM.pdf'.format(self.planet.Mp,
                                                                        self.planet.XFe, self.planet.FeM), bbox_inches="tight")
            plt.show()

        plt.figure(figsize=(5, 4))
        plt.plot(self.planet.time_vector[1:], self.QC[1:],
                 label='Secular cooling', color='deepskyblue')
        plt.plot(self.planet.time_vector[1:], self.QL[1:],
                 label='Latent heat', color='firebrick')
        plt.plot(self.planet.time_vector[1:], self.QX[1:],
                 label='Gravitational heat', color='coral')
        plt.plot(self.planet.time_vector[1:], self.QL[1:]+self.QC[1:] +
                 self.QX[1:], label='Total ($Q_{\mathrm{CMB}}$)', color='mediumblue')
        plt.xlabel('Time (years)')
        ...
        plt.xlim([0, 5e9])
        plt.ylim([0, 5.4e13])
        plt.savefig(plots_folder + 'Energy_balance_{}ME_{}XFe_{}FeM.pdf'.format(
            self.planet.Mp, self.planet.XFe, self.planet.FeM), bbox_inches="tight")
        plt.show()
        ...
        plt.savefig(plots_folder + 'QCMB_TCMB_{}ME_{}XFe_{}FeM.pdf'.format(
            self.planet.Mp, self.planet.XFe, self.planet.FeM), bbox_inches="tight")
        plt.show()

        plt.figure(figsize=(5, 4))
        plt.plot(
            self.planet.time_vector[1:], self.F_th[1:], label='Temperature', color='tomato')
        plt.plot(self.planet.time_vector[1:], self.F_X[1:],
                 label='Composition', color='mediumseagreen')
        plt.xlabel('Time (years)')
        ...
        plt.xlim([0, 5e9])
        plt.semilogy()
        plt.savefig(plots_folder + 'Fluxes_{}ME_{}XFe_{}FeM.pdf'.format(
            self.planet.Mp, self.planet.XFe, self.planet.FeM), bbox_inches="tight")
        plt.show()
        ...
        plt.plot(self.planet.time_vector, (self.Bc * 1e6),
                 label='CMB', color='tomato')
        plt.plot(self.planet.time_vector, (self.Bs * 1e6),
                 label='Surface', color='mediumseagreen')
        plt.xlabel('Time (years)')
        ...
        ax1.plot(self.planet.time_vector[1:],
                 self.M_ratio[1:], color='crimson')
        ...
        #plt.suptitle('$M=$ %.1f $M_{\oplus}$, $C_{\mathrm{Fe}}=$ %.0f wt %%' %(np.float(self.planet.Mp),np.float(self.planet.XFe)))
        plt.subplots_adjust(wspace=0.4)
        plt.savefig(plots_folder + 'MField_{}ME_{}XFe_{}FeM.pdf'.format(
            self.planet.Mp, self.planet.XFe, self.planet.FeM), bbox_inches="tight")
        plt.show()
...
# ------------------------------------------------------------------------------------------------------------------- #
    ...
    def update_noic(self, T, Delta_time, qcmb):
        ...
        PC = (-4*np.pi/3*self.planet.rho_0 *
              self.planet.CP*self.planet.L_rho**3*fC)
    ...
    def update_ic(self, r_IC, Delta_time, qcmb, P_IC, S_t, ratio=1):
        ...
        # maximum Tmelt depression in Morard (2012):
        if self.planet.S != 0. and self.T_liquidus_core(P_IC, 0.) - self.T_liquidus_core(P_IC, S_t) < 1500.:
...
# ------------------------------------------------------------------------------------------------------------------- #
    ...
    def dTL_dr_IC(self, x):
        ...
        def fun(x):
            ...
            fC = (r_OC/L_rho)**3. * (1. - 3. / 5. * (0. + 1.) * (r_OC/L_rho)
                                     ** 2. - 3. / 14. * (0. + 1) * (2 * A_rho - 0.) * (r_OC/L_rho)**4.)
    ...
    def M_OC(self, r):
        ...
        if r == self.planet.r_OC:
            ...
            mass = 4./3. * np.pi * self.planet.rho_0 * self.planet.L_rho**3 * \
                (self.fC(self.planet.r_OC/self.planet.L_rho, 0.) -
                 self.fC(r/self.planet.L_rho, 0.))
        return mass
    ...
    def pressure_diff(self, r):
        ...
        K0 = (2./3. * np.pi * self.planet.L_rho **
              2 * self.planet.rho_0**2 * GC)/1e9
        factor = (r**2)/(self.planet.L_rho**2) - \
            (4.*r**4)/(5.*self.planet.L_rho**4)
        return -K0*factor
    ...
    def _Bc(self, rho_OC, F_th, F_X, r_IC):
        if (F_th + F_X) < 0. or (self.planet.r_OC-r_IC) == 0.:
            ...
            Bc = beta * np.sqrt(rho_OC * mu_0) * \
                ((F_th+F_X)*self.planet.r_OC)**(1./3.)
        return Bc
    ...
    def _magn_moment(self, F_th, F_X, r_IC, Q_CMB, QC_ad):
        ...
        u = 1.3 * ((self.planet.r_OC-r_IC)/7.29e-5)**(1./5.) * \
            (F_th + F_X)**(2./5.)
        Rem = (u*(self.planet.r_OC-r_IC))/eta_m
        if (F_th + F_X) < 0. or Rem < 40.:
            M = 0.
        else:
            M = 4 * np.pi * self.planet.r_OC**3 * beta * \
                np.sqrt(self.planet.rho_0/mu_0) * \
                ((F_th + F_X)*(self.planet.r_OC-r_IC))**(1./3.)
        return M

    def _M_Aubert(self, r, F_th, F_X, Q_CMB, QC_ad):
        """Magnetic moment (Aubert et al.,2009)"""
        u = 1.3 * ((self.planet.r_OC-r)/7.29e-5)**(1./5.) * \
            (F_th + F_X)**(2./5.)
        ...
        if (F_th + F_X) < 0. or Rem < 40.:
            ...
            gam = (3*(self.planet.r_OC-r)**2.)/(2*(self.planet.r_OC**3. - r**3.)*self.planet.r_OC) * (fi*(3./5. * (self.planet.r_OC**5. - r**5.) /
                                                                                                          (self.planet.r_OC**3. - r**3.)-r**2.) + (1-fi)*(self.planet.r_OC**2. - 3./5.*(self.planet.r_OC**5.-r**5.)/(self.planet.r_OC**3.-r**3.)))
            D = self.planet.r_OC-r
            RaQ = (self.planet.gc * (F_th+F_X)) / \
                (4*np.pi*self.planet.rho_0 * (D)**4.)
            p = gam*RaQ
            f_ohm = 1.
            c1 = 1.65
            asp = r/self.planet.r_OC
            M = (4*np.pi*self.planet.r_OC**3.)/(np.sqrt(2.*mu_0)) * (c1 * np.sqrt(f_ohm)
                                                                     * (p)**(0.34)*np.sqrt(self.planet.rho_0)*D)/(7.3 * (1.-asp)*(1.+fi))
        return M
    ...
    def _F_X(self, r, drIC_dt, S, r_IC, S_eut):
        ...
        g = 4. * np.pi/3. * GC * self.planet.rho_0 * r_IC*(1.-3./5. * ((r_IC)**2)/(
            (self.planet.L_rho)**2) - 3. * self.planet.A_rho/7. * ((r_IC)**4.)/((self.planet.L_rho)**4.))
...
class Evolution_Labrosse2015(Evolution):    
    ...
    def dTL_dr_IC(self, r):
        result = -self.planet.K_c * 2.*self.planet.dTL_dP * r / self.planet.L_rho**2. \
            + 3. * self.planet.dTL_dchi * self.planet.chi0 * r**2. / \
            (self.planet.L_rho**3. * self.fC(self.planet.r_OC / self.planet.L_rho, 0.))
        return result
...
class Rocky_Planet(Planet):
    ...
    def __init__(self, Mp, XFe, FeM, S, data_folder="../data/", with_DTcmb=True, file_qcmb_provided=True, qcmb=None):
        ...
        if file_qcmb_provided:
            ...
            self.qcmb = qcmb
        self.parameters()
        print("Planet iniatialized. Mass is {}, core fraction is {}wt%".format(
            Mp, XFe/100))

    def structure_file_name(self):
        name = "M_ {:.1f}_Fe_{:.0f}.0000_FeM_{:2.0f}.0000.yaml".format(
            self.Mp, self.XFe, self.FeM)
        if self.with_DTcmb:
    ...
    def qcmb_file_name(self):
        name = "res_t_HS_Tm_Tb_qs_qc_M{:02d}_Fe{:02d}_#FeM{:02d}.res".format(
            int(10*self.Mp), int(self.XFe), int(self.FeM))
    ...
    def parameters(self):
        ...
        if self.qcmb_file != None:
            qcmb_ev = pd.read_csv(self.qcmb_file, skipinitialspace=True, sep=" ", index_col=False, skiprows=[
                                  0], names=["time", "H_rad", "T_um", "T_cmb", "q_surf", "qcmb"])
        # qcmb_ev.columns = ["time", "H_rad", "T_um","T_cmb","q_surf","qcmb"]
            ...
            if self.qcmb == None:
                print(
                    "A value for QCMB is needed. Please define the variable qcmb or provide a filename.")
            else:
                N = 500
                self.time_vector = pd.Series(
                    np.linspace(0, 5e9, N))  # 5Gyears history
