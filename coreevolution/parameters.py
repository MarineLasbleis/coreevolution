...
# for fitting the density with a function
...
def read_data_profiles(filename, core=False):
    names = ["g(m/s^2)", "p(GPa)", "rho(kg/m^3)", "r(m)", "T(K)", "oups", "Cp(J/kgK)", "alpha(10^-5 1/s)",
             "Gruneisen(1)", "KT(GPa)", "KS(GPa)", "G(GPa)", "ElCond (Siemens)", "Material-Parameter"]
...
def find_Lrho_Arho(profile):
    ...
    radius = profile["r(m)"]
    # initial values from Labrosse 2015 (for the Earth)
    initial_guess = 12502, 8039e3, 0.484
...
def Earth():
    ...
    param = {
        ...
        #
        # Thermal expansion coefficient at center (K-1)
        "alpha_c": 1.3e-5,
        ...
        # Entropy of crystallization (Jkg-1K-1)
        "DeltaS": 127.,
        "H": 0.,               # Radioactivity
        # Thermal conductivity at center (Wm-1K-1)
        "k_c": 163.,
        ...
        # Compositional dependence of liquidus temperature (K)
        "dTL_dchi": -21.e3,
        # Pressure dependence of liquidus temperature (Pa-1)
        "dTL_dP": 9.e-9,
...
def calculate_parameters(filename, verbose=False):
    ...
    newstr = ''.join((ch if ch in '0123456789.' else ' ')
                     for ch in os.path.basename(filename[:-4]))
    ...
    param["rho_0"], param["L_rho"], param["A_rho"] = find_Lrho_Arho(
        core)  # rho_0 is the density at the center
    ...
    param["r_IC_0"] = find_r_IC_adiabat(
        param["rho_0"], param["L_rho"], param["A_rho"], P0, param["T0"], param["gamma"], S=0.)
    param["r_IC_005"] = find_r_IC_adiabat(
        param["rho_0"], param["L_rho"], param["A_rho"], P0, param["T0"], param["gamma"], S=0.05)
    param["r_IC_011"] = find_r_IC_adiabat(
        param["rho_0"], param["L_rho"], param["A_rho"], P0, param["T0"], param["gamma"], S=0.11)
    ...
    # Earth's bulk modulus at the center (Labrosse+2015)
    param["K_c"] = 1403.e9
    return param, core
...
def write_parameter_file(filename, fig=False, folder="", verbose=False):
    ...
    output_filename = folder + \
        "M_ {:.1f}_Fe_{:.0f}.0000_FeM_{:2.0f}.0000.yaml".format(Mp, XFe, FeM)
    ...
    if fig:
        ...
        figure(core, ax3)
        ax3[1, 0].plot(radius[::100]/1e3, density_Labrosse2015(radius[::100],
                                                               param["rho_0"], param["L_rho"], param["A_rho"]), '+')
        ax3[0, 1].plot(radius[::100]/1e3, gravity(radius[::100],
                                                  param["rho_0"], param["L_rho"], param["A_rho"]), '+')
        ax3[1, 1].plot(radius[::100]/1e3, pressure_diff(radius[::100],
                                                        param["rho_0"], param["L_rho"], param["A_rho"])+P0, '+')
        ax3[0, 0].plot(radius[::100]/1e3, T_adiabat(radius[::100], param["L_rho"],
                                                    param["A_rho"], param["T0"], param["gamma"]), '+', label="fit")
        ax3[0, 0].plot(radius/1e3, T_liquidus_core(pressure_diff(radius,
                                                                 param["rho_0"], param["L_rho"], param["A_rho"])+P0), label="Melting T")
        ax3[0, 0].plot(np.array([param["r_IC_0"], param["r_IC_0"]]) /
                       1e3, [core["T(K)"].iloc[-1], core["T(K)"].iloc[0]])
        ax3[0, 0].legend()
    # param["Mp"], param["XFe"], param["FeM"], param["rho_0"], param["L_rho"], param["A_rho"]
    return param
...
def explore_all_create_yaml(folder, fig=False, verbose=False):
    ...
    for file in files:
        if file.split('/')[-1] != "data_IS.res":
            ...
            Mp, XFe, FeM, rho, L, A = param["Mp"], param["XFe"], param[
                "FeM"], param["rho_0"], param["L_rho"], param["A_rho"]
    ...
    if fig:  # TODO to be changed (the names are not correct)
        ...
        data = pd.read_csv(all_files, skipinitialspace=True,
                           sep=" ", names=names)
